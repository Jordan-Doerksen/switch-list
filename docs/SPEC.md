...
- **Status (2026‑06‑18):** P0 + clear/dig + #7 + car types + **kicking + loads/empties** playable & verified. **8 authored puzzles**, all solver‑verified + kick/load‑rule tests pass. Runtime generation abandoned. Pending mechanics to flesh out before the big puzzle push: **road jobs (P6)**, **manifest order / blocked outbound (P7)**. Then expand to ~4 puzzles per layer. GitHub Pages: enabled (push to go live).
