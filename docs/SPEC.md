...
- **Road jobs + manifest order (P6+P7, BUILT 2026‑06‑18) — proven as law.** Outbound goal can be **`ordered: true`** (exact throat→deep manifest order / blocking — `checkWin` does ordered compare; the solver builds in order) and **`depart: true`** (a deliberate **Depart ▸** action: enabled once the consist is assembled, the call wins). Inbound set‑out is pre‑placed (logic works the post‑arrival yard) + described in the work order. *(The inbound‑arrival and departure CINEMATICS are deferred flavor — the provable logic is the ordered build + depart; animations don't verify in the backgrounded preview anyway.)*
- **Status (2026‑06‑18):** P0 + clear/dig + #7 + car types + kicking + loads/empties + **road jobs/manifest + depart** playable & verified. **9 authored puzzles**, all solver‑verified + kick/load‑rule tests pass. Runtime generation abandoned. **All curriculum mechanics now fleshed out & proven** — next is the puzzle push (≈4 per layer) + the arrival/departure cinematics polish. GitHub Pages: enabled (push to go live).
