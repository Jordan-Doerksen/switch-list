...
function clone(s) {
  ...
  return { tracks, pos, type: s.type, loaded: s.loaded, kickable: s.kickable, kickLimit: s.kickLimit, engine: s.engine.slice(), lead: (s.lead || []).slice(), secured, lined, out: s.out, moves: s.moves, joints: s.joints, dist: s.dist, msg: '', won: s.won };
}
